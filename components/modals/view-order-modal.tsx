...
const ViewOrderModal = () => {
	...
	const imageLoager: ImageLoader = ({ src, width }) => {
		return `/uploads/goods/${src}?w=${width}`;
	};
					...
					<div>
						...
						<div className="p-2 flex flex-col gap-1">
							...
							<span className="flex text-sm gap-2">
								<p>Частный дом?</p>
								<span className="flex-1 border-dashed border-b-[1px] border-zinc-300 mb-1" />
								<p>{orderData?.is_private_house ? 'да' : 'нет'}</p>
							</span>
							{!orderData?.is_private_house && (
								<span className="flex text-sm gap-2">
									<p>Подъезд</p>
									<span className="flex-1 border-dashed border-b-[1px] border-zinc-300 mb-1" />
									<p>{orderData?.entrance}</p>
								</span>
							)}
							{!orderData?.is_private_house && (
								<span className="flex text-sm gap-2">
					...
					<div>
						...
						<div className="py-2 flex flex-col gap-1">
							...
							<span className="flex text-sm gap-2">
								<p>Сумма заказа</p>
								<span className="flex-1 border-dashed border-b-[1px] border-zinc-300 mb-1" />
								<p>{calcTotalPrice(orderData?.products)} ₽</p>
							</span>
						</div>
					</div>
				</div>
